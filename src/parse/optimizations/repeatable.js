...
const { Constant, Value } = require("../../types/value");
...
module.exports = (state, { tokens, i }) => {
    ...
    if (tokens.length > i + 1 && tokens[i].offset == tokens[i + 1].offset && tokens[i + 1].instr == tokens[i].instr && tokens[i].is(Rules.Repeatable)) {
        ...
        state.index = i;
        state.result.push(new Instruction(tokens[i].instr, new Value(new Constant(num)), tokens[i].offset, tokens[i].line, tokens[i].char));
